...
use fprime_frame::{Deframer, FpDeframer, FpFramer, Framer};

/// Type-erased framer with shared interior mutability so the comm task can
/// hold one across `await` points.
pub type SharedFramer = std::sync::Arc<Mutex<Box<dyn Framer>>>;
pub type SharedDeframer = std::sync::Arc<Mutex<Box<dyn Deframer>>>;

pub fn shared_framer<F: Framer + 'static>(f: F) -> SharedFramer {
    std::sync::Arc::new(Mutex::new(Box::new(f)))
}

pub fn shared_deframer<D: Deframer + 'static>(d: D) -> SharedDeframer {
    std::sync::Arc::new(Mutex::new(Box::new(d)))
}

/// Build the F´ DEADBEEF/CRC32 framer pair.
pub fn fprime_pair() -> (SharedFramer, SharedDeframer) {
    (shared_framer(FpFramer), shared_deframer(FpDeframer::new()))
}
...
///
/// The framer/deframer pair determines the wire protocol — pass
/// [`fprime_pair`] for the legacy F´ format, or build a CCSDS chain.
pub fn spawn(addr: SocketAddr, mode: Mode, framer: SharedFramer, deframer: SharedDeframer) -> Comm {
    let (dl_tx, dl_rx) = mpsc::unbounded_channel();
    let (ul_tx, ul_rx) = mpsc::unbounded_channel();
    let (st_tx, st_rx) = mpsc::unbounded_channel();

    tokio::spawn(async move {
        if let Err(e) = run(addr, mode, framer, deframer, dl_tx, ul_rx, st_tx).await {
...
async fn run(
    ...
    loop {
        ...
        let session_result =
            run_session(stream, framer.clone(), deframer.clone(), dl_tx2, &mut ul_rx).await;
...
async fn run_session(
    ...
    let writer = async {
        loop {
            ...
            let framed = {
                let mut f = writer_framer.lock().expect("framer poisoned");
                match f.frame(&payload) {
                    Ok(f) => f,
                    Err(e) => {
    ...
    let reader_deframer = deframer.clone();
    let reader = async {
        ...
        loop {
            ...
            let mut d = reader_deframer.lock().expect("deframer poisoned");
            d.push(&buf[..n]);
            while let Some(p) = d.pop() {
                if dl_tx.send(p).is_err() {
                    return Ok::<(), anyhow::Error>(());
                }
            }
        }
